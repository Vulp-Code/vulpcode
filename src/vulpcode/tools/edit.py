...
from vulpcode.tools._edit_validators import validate_after_edit
...
def _apply_edit(text, old, new, replace_all):
    ...
    if occurrences == 0:
        return text, 0, "old_string not found"
    if occurrences > 1:
        return text, 0, f"old_string is not unique ({occurrences} occurrences). Add more context or set replace_all=True."
    return text.replace(old, new, 1), 1, None


def _snippet_around_change(new_text, search, context_lines=3):
...
@tool(name="Edit", description="Replace exact occurrences of old_string with new_string in a file.", requires_confirm=True)
class EditTool(Tool):
    ...
    async def run(self, args: BaseModel) -> ToolResult:  # type: ignore[override]
        ...
        new_text, count, err = _apply_edit(text, args.old_string, args.new_string, args.replace_all)
        if err is not None:
            return ToolResult(error=err, is_error=True)
        revalidation_err = validate_after_edit(str(path), new_text)
        if revalidation_err:
            return ToolResult(error=f"Re-validation failed after edit: {revalidation_err}", is_error=True)
        hits = scan_secrets(new_text)
        if hits:
        ...
        return ToolResult(output=f"Applied {count} edit(s) to {path}\n{snippet}", metadata={"file_path": str(path), "edits_applied": count})


@tool(name="MultiEdit", description="Atomically apply multiple edits to a single file.", requires_confirm=True)
class MultiEditTool(Tool):
    ...
    async def run(self, args: BaseModel) -> ToolResult:  # type: ignore[override]
        ...
        for i, op in enumerate(args.edits, start=1):
            text, count, err = _apply_edit(text, op.old_string, op.new_string, op.replace_all)
            if err is not None:
                return ToolResult(error=f"Edit #{i} failed: {err}", is_error=True)
            total += count
        revalidation_err = validate_after_edit(str(path), text)
        if revalidation_err:
            return ToolResult(error=f"Re-validation failed after edit: {revalidation_err}", is_error=True)
        ...
        try:
            ...
            return ToolResult(error=f"Failed to write: {exc}", is_error=True)
        return ToolResult(output=f"Applied {total} edit(s) across {len(args.edits)} operations to {path}", metadata={"file_path": str(path), "edits_applied": total, "ops": len(args.edits)})
